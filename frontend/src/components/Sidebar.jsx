...
export default function Sidebar({ activeTab, setActiveTab, sidebarCollapsed, userRole }) {
  ...
  const filteredMenuItems = menuItems.filter(item => {
    if (userRole === 'Employer') {
      return ['employer', 'inventory', 'teambuilder', 'analytics'].includes(item.id);
    }
    return ['professional'].includes(item.id);
  });
  ...
  return (
    <aside className={`sidebar ${sidebarCollapsed ? 'collapsed' : ''}`}>
      ...
      <div className="sidebar-footer">
        <button className="menu-link" onClick={() => alert('Settings - Prototype Shell')}>
